...
get_single_year <- function(year = 2022, num_vars = c("AGEP", "PWGTP"), 
    ...
    #State Check
    if (geography == "State" && as.numeric(geography_level) %in% 0:50)
       {geography <- "State"

    #Division Check
    } else if (geography == "Division" && as.numeric(geography_level) %in% 0:9)
        
    {geography <- "Division"

    #Region Check
    } else if (geography == "Region" && as.numeric(geography_level) %in% c(1,2,3,4,9))
    {geography <- "Region"

    } else
    stop("Valid State values are between 01 and 50\n Valid Division values are between 0 and 9\n Valid Region values are 1,2,3,4,9")

    
    #Create geography argument
