...
get_single_year <- function(year = 2022, num_vars = c("AGEP", "PWGTP"), 
    ...
    # # #R1.4A - Validate Specified Geography (This code is no longer needed)
    # if (geography == "All") { 
    #     geography <- ""
    ...
    # #R1.4B - Validate Specified Geography Level
    if (geography == "State" && grepl(c(paste0("0", 1:9), as.character(c(10:50))),geography_level) == TRUE){
    geography <- "State:"}
    else {stop("Valid State values are between 01 and 50")
    }

    if (geography == "Division" && grepl(as.character(c(0:9)),geography_level) == TRUE){
    geography <- "Division:"}
    else {stop("Valid Division values are between 0 and 9")
    }

    if (geography == "Region" && grepl(as.character(c(1,2,3,4,9)),geography_level) == TRUE){
    geography <- "Region:"}
    else {stop("Valid Region values are 1,2,3,4,9")
    }
    ...
    df <- df |> mutate(JWDP = str_pad(JWDP, width = 3, side = "left", pad = "0"))
        #Checks if "JWDP" is one of the variables in num_vars. If not, the block is skipped. 
        #Pads the JWDP column in df with leading zeros so all values are 3 characters wide (e.g., "7" becomes "007").
        ...
        for (i in 1:nrow(df)) {
            ...
            #If the JWDP code is "000" (which means "Not applicable"), sets it to NA.
                ...
                #jwdp_map[[ df$JWDP[i] ]]: Looks up the value in jwdp_map using the JWDP code from row i of df. 
                #This value is a string like "5:00 a.m. to 5:29 a.m.". 
                #strsplit(..., " to "): Splits that string into two parts at " to ", resulting in a vector like c("5:00 a.m.", "5:29 a.m.").
                #[[1]]: Extracts the first (and only) element of the list returned by strsplit, producing a character vector.
                #str_remove_all("\\."): Removes all periods from both start and end time strings, so "5:00 a.m." becomes "5:00 am".
                #str_to_upper(): Converts both strings to uppercase, so "5:00 am" becomes "5:00 AM".
        ...
        #After the loop, converts the JWDP column to HMS (hour-minute-second) time format.
        #as.numeric(JWDP): Converts the JWDP column to numeric values (if it isn’t already).
        #hms::as_hms(...): Converts those numeric values to time objects in the HMS (hours:minutes:seconds) format.
        #mutate(JWDP = ...): Updates the JWDP column in the data frame df with these new time values.
...
#    cat_var_maps <- list()
    for (variable in cat_vars) {
        ...
        #names(var_map): Gets the vector of code names (e.g., "01", "02", "0", "10").
        #str_replace(names(var_map), "^0+(?!$)", ""): Uses a regular expression to remove one or more zeros at the start of each name, but not if the name is just "0". The result: "01" becomes "1", "002" becomes "2", but "0" stays "0".
...
summary.census <- function(tibble, num_vars = NULL, cat_vars = NULL) {
    ...
    if (!is.null(num_vars)) {
        ...
        #myear_tibble |> ...: Pipes the data frame into the next operation.
        #summarise(...): Collapses the data frame to a single row with summary statistics.
        #across(all_of(num_vars) & !all_of("PWGTP"), ...): Applies the function to all columns in num_vars except "PWGTP".
        #For each column, create a new column in the output with the name given by .names = "{.col}_mean" (e.g., AGEP_mean).
        #~ sum(.x * PWGTP) / sum(PWGTP): Calculates the weighted mean for each variable, using PWGTP as the weight.
        #.names = "{.col}_mean": Names the new columns as the original column name plus _mean.
       
        sd_tibble <- tibble |> summarise(across(all_of(num_vars) & !all_of("PWGTP"),
                        ~ sqrt(sum(.x^2 * PWGTP) / sum(PWGTP) - mean(.x)^2 ), .names = "{.col}_sd"))
    }
        #myear_tibble |> Pipes the data frame into the next operation.
        #summarise(...): Collapses the data frame to a single row with summary statistics.
        #across(all_of(num_vars) & !all_of("PWGTP"), ...): Applies the function to all columns in num_vars except "PWGTP". 
        #For each column, create a new column in the output with the name given by .names = "{.col}_sd" (e.g., AGEP_sd).
        #sqrt(sum(.x^2 * PWGTP) / (sum(PWGTP) - mean(.x)^2 )): 
        #Calculates the weighted standard deviation for each variable, using PWGTP as the weight.
        #.names = "{.col}_sd": Names the new columns as the original column name plus _sd.
    
    
    #check if cat_vars is TRUE
    if(!is.null(cat_vars)) {
    #summarize frequency counts    
        freq_tibble <- tibble |> group_by(across(all_of(cat_vars))) |> summarize(Freq=n())       #Checks if cat_vars is not NULL.
    # Groups myear_tibble by all columns in cat_vars.
    #Summarizes the frequency (count) of each group.
    ...
    if (is.null(num_vars) && is.null(cat_vars)) {
        ...
        sd_tibble <- tibble |> summarise(across(where(is.numeric) & !all_of("PWGTP"),
                            ~ sqrt( sum(.x^2 * PWGTP) / sum(PWGTP) - mean(.x)^2 ), .names = "{.col}")) 
