...
get_single_year <- function(year = 2022, num_vars = c("AGEP", "PWGTP"), 
    ...
    #Division Check
    } else if (geography == "Division" && (as.numeric(geography_level) %in% 0:9) == FALSE)
    {stop("Valid Division values are between 0 and 9")

    #Region Check
    } else if (geography == "Region" && (as.numeric(geography_level) %in% c(1,2,3,4,9) == FALSE))
    {stop("Valid Region values are 1,2,3,4,9")
...
syear_test <- get_single_year(num_vars = c("PWGTP","AGEP", "JWDP"),
                       ...
                       geography = "Region", geography_level = c("5"))
